...
if (@$_POST) {
    ...
    if ($hasil_cek) {
        ...
        $update_psi = "UPDATE presensikelas SET psi = '$psi' WHERE mulai_jamke = '$mulai_jamke' AND nis = '$nis' AND tanggal LIKE '%$tanggal%'";
        $update_query = mysqli_query($konek, $update_psi);

        if ($update_query) {
            echo "Berhasil di Update \npsi : '" . $psi . "'";
        } else {
            echo "Gagal update psi";
        }
    } else {
