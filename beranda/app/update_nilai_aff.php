...
if (@$_POST) {
    ...
    if ($hasil_cek) {
        ...
        $update_aff = "UPDATE presensikelas SET aff = '$aff' WHERE mulai_jamke = '$mulai_jamke' AND nis = '$nis' AND tanggal LIKE '%$tanggal%'";
        $update_query = mysqli_query($konek, $update_aff);

        if ($update_query) {
            echo "Berhasil di Update \naff : '" . $aff . "'";
        } else {
            echo "Gagal update aff";
        }
    } else {
