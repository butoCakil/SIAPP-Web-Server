...
$nama_guru = "";
$selisih_waktu = 0;
$selisih_waktu_selesai = 0;
// if (!isSafeInput($nokartu) || !isSafeInput($idchip) || !isSafeInput($nodevice)) {
//     die("Program dihentikan karena karakter mencurigakan ditemukan. di input");
// }
...
if ($nokartu) {
    ...
    if ($idchip_clean) {
        ...
        mysqli_stmt_bind_param($stmt_select_reg_device, "s", $idchip_clean);
        ...
        if ($row = mysqli_fetch_assoc($result_select_reg_device)) {
            ...
            if ($nodevice == $hasil_nodevice) {
                ...
                // echo("mode:" . $mode . ", wa:" . $wa . ", wta:" . $wta . ", wtp:" . $wtp . ", wp:" . $wp . ", jam:" . $jam . ", tgl:" . $tanggal . ", tms" . $timestamp);
                ...
                $query_select_dataguru = "SELECT nama, nip FROM dataguru WHERE nokartu = ?";
                ...
                $query_select_datasiswa = "SELECT nama, nis FROM datasiswa WHERE nokartu = ?";
                ...
                if ($nama_gtk != "" || $nama_siswa != "") {
                    ...
                    if ($hasil_kode_device == "GATE") {
                        ...
                        if ($harilibur == false) {
                            ...
                            if ($nama_gtk) {
                                ...
                                if ($tglakhir < $tanggal) {
                                    ...
                                    mysqli_stmt_bind_param($stmt_update_dataguru, "s", $nokartu_clean);
                                ...
                                if ($tglakhir < $tanggal) {
                                    ...
                                    mysqli_stmt_bind_param($stmt_update_datasiswa, "s", $nokartu_clean);
                            ...
                            if ($mode == 1) {
                                ...
                                mysqli_stmt_bind_param($stmt_select_datapresensi, "ss", $nokartu_clean, $tanggal);
                                ...
                                if (!$hasilCek) {
                                    ...
                                    if ($jam <= $waktumasuk) {
                                        ...
                                        if ($t_waktu_telat_siswa) {
                                            ...
                                            // echo ("telat: " . $t_waktu_telat_siswa . ", poin:" .$poin_siswa . ", no:" . $nokartu_clean); die;

                                            $query_update_datasiswa = "UPDATE datasiswa SET t_waktu_telat = ?, poin = ? WHERE nokartu = ?";
                                            $stmt_update_datasiswa = mysqli_stmt_init($konek);
                                            mysqli_stmt_prepare($stmt_update_datasiswa, $query_update_datasiswa);
                                            mysqli_stmt_bind_param($stmt_update_datasiswa, "sss", $t_waktu_telat_siswa, $poin_siswa, $nokartu_clean);
                                            mysqli_stmt_execute($stmt_update_datasiswa);
                                            ...
                                            $stmt_update_dataguru = mysqli_stmt_init($konek);
                                            mysqli_stmt_prepare($stmt_update_dataguru, $query_update_dataguru);
                                            mysqli_stmt_bind_param($stmt_update_dataguru, "sss", $t_waktu_telat_guru, $poin_guru, $nokartu_clean);
                                            mysqli_stmt_execute($stmt_update_dataguru);
                                    ...
                                    mysqli_stmt_bind_param($stmt_insert_datapresensi, "ssssssssssss", $nokartu_clean, $nama, $noReg, $info, $foto, $jam, $ket, $selisihWaktu, $tanggal, $keterangan, $timestamp, $kode);
                                ...
                                if (!$data) {
                                    ...
                                    mysqli_stmt_bind_param($stmt_update_datapresensi, "ssssssss", $jam, $ket, $selisihWaktu, $keterangan, $timestamp, $nokartu_clean, $tanggal);
                        ...
                        // echo ("hasil cek:" . $hasilCek . ", nokartu:" . $nokartu_clean . ", ket:" . $ket . ", pesan:" . $pesan);
                        ...
                        if (mysqli_stmt_prepare($stmt, "SELECT * FROM presensiEvent WHERE nokartu = ? AND tanggal = ?")) {
                            ...
                            if ($row = mysqli_fetch_assoc($result)) {
                                ...
                                if ($row['mulai'] && $row['selesai']) {
                                    ...
                                    $selisih_waktu_selesai = $waktu_sekarang - $waktu_selesai;

                                    if ($selisih_waktu_selesai > 300) {
                                    ...
                                    if ($selisih_waktu > 240) { // 300 detik = 5 menit
                                        $stmt = mysqli_stmt_init($konek);
                                        if (mysqli_stmt_prepare($stmt, "UPDATE presensiEvent SET selesai = ? WHERE nokartu = ?")) {
                                            mysqli_stmt_bind_param(
                                                $stmt,
                                                "ss",
                                                $jam,
                        ...
                        if (mysqli_stmt_prepare($stmt, "INSERT INTO `daftarijin` (`nokartu`, `ni`, `nama`, `info`, `tanggalijin`, `kode`) VALUES (?, ?, ?, ?, ?, ?)")) {
                            mysqli_stmt_bind_param(
                                $stmt,
                                "ssssss",
                                $nokartu_clean,
                        ...
                        $query_select_jadwalkbm = "SELECT info, kelas, kelompok, tingkat, jur, nick, taggal, mulai_jamke, sampai_jamke FROM jadwalkbm WHERE ruangan = ? AND tanggal = ?";
                        mysqli_stmt_prepare($statement_select_jadwalkbm, $query_select_jadwalkbm);
                        mysqli_stmt_bind_param($statement_select_jadwalkbm, "ss", $hasil_kode_device, $tanggal);
                        ...
                        if ($row = mysqli_fetch_assoc($result_select_jadwalkbm)) {
                            ...
                            mysqli_stmt_bind_param($statement_select_dataguru, "s", $jadwal_nick);
                            ...
                            mysqli_stmt_bind_param($statement_insert_presensikelas, "ssssssssss", $nokartu_clean, $ni, $nama, $hasil_kode_device, $jadwal_kelas, $jadwal_mulai_jamke, $jadwal_sampai_jamke, $jadwal_nick, $nama_guru, $tanggal);
                            ...
                            $statement_select_jadwalkbm = mysqli_stmt_init($konek);
                            mysqli_stmt_prepare($statement_select_jadwalkbm, $query_select_jadwalkbm);
                            mysqli_stmt_bind_param($statement_select_jadwalkbm, "s", $hasil_kode_device);
                            mysqli_stmt_execute($statement_select_jadwalkbm);
...
if (!isSafeInput2($ip_a)) {
    die("Program dihentikan karena karakter mencurigakan pada IPa.");
}
...
mysqli_stmt_bind_param($stmt_insert_tempreq, "ssss", $clientIP, $jsonResponse, $pesan, $detail);
...
// KETERANGAN KODE
// 404  = ERROR. Method Not Allowed. (_...)
// 405  = ERROR. Method Not Allowed. (_....)
// 406  = ERROR. Not Acceptable. ID CHIP TIDAK TERDAFTAR (_.....)
// 407  = AKSES DITOLAK! DEVICE TIDAK SESUAI (_......)
// IDTT = Kartu ID ini belum terdaftar (._..)
// HLTM = Hari ini Libur (._.)
// TBPS = Tidak bisa melakukan presensi sekarang. (._._..)
// SAPP = Sudah melakukan presensi pulang (...)
// PLAW = Pulang lebih awal (.._)
// PPBH = Presensi Pulang berhasil (..)
// PPPP = $nama . $keterangan (...)
// SMPM = Anda Sudah melakukan Presensi masuk (...)
// MMMM = $nama . $keterangan (...)
// BMPM = Berhasil melakukan Presensi (..)
// BMPE = Berhasil melakukan Presensi Kegiatan (..)